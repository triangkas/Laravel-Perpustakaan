<x-app-layout>
            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900">
                    <div class="flex items-center w-full">
                        <x-secondary-button onclick="window.location.href='{{ route('buku.add') }}'">
                            {{ __('Tambah') }}
                        </x-secondary-button>

                        <form method="GET" action="{{ route('buku.show') }}" class="ml-auto">
                                ...
                                @foreach($listData as $key => $rows)
                                    <tr>
                                        <td align="center" width="250">
                                            <x-secondary-button class="ms-3" onclick="window.location.href='{{ route('buku.update', $rows->id) }}'">{{ __('Edit') }}</x-danger-button>
                                        ...
                                        <td align="left">{{ $rows->judul_buku }}</td>
                                        <td align="left">{{ $rows->penerbit }}</td>
                                        <td align="left">{{ $rows->dimensi }}</td>
                                        <td align="center">{{ $rows->stock }}</td>
                                    </tr>
                                @endforeach
                            @else
    ...
    @endsection
    <x-modal-delete url="{{ route('buku.delete') }}" />
</x-app-layout>